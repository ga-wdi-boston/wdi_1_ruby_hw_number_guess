...
num_guesses = 0
puts "#{user_name.empty? ? 'You' : ', ' + user_name + ', you'} will be allowed #{max_guesses} chances to guess a number between #{guess_range.first} and #{guess_range.last}."
...
begin
  ...
  puts "\n\nLets roll!! Please enter a number between #{guess_range.first} and #{guess_range.last} =>"
  ...
  if !guess_range.include?(user_guess)
    ...
    puts "\n\nCongratulations#{user_name.empty? ? '' : ' ' + user_name}!  You got the number in #{num_guesses + 1} #{num_guesses  == 1 ? 'guess' : 'guesses'}!!"
  else
    higher = user_guess > guess_this_number
    guesses_left -= 1
    num_guesses = max_guesses - guesses_left
    puts "Sorry#{user_name.empty? ? '' : ' ' + user_name}, the number #{user_guess} was too #{higher ? 'high' : 'low'}."
    if guesses_left == 0
      puts "So sorry#{user_name.empty? ? '' : ' ' + user_name}, you've run out of guesses. The number we are trying to guess is #{guess_this_number}"
    else
      puts "Try again - you've got #{guesses_left} #{guesses_left  == 1 ? 'guess' : 'guesses'} left!"
    end
  end
